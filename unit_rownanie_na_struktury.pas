...
unit  unit_rownanie_na_struktury;
...
function typ_elementu(tymczasowy:string):Typy;
    var
        ...
        dlugosc:      integer;
        i:            integer;
        licz_funk:    integer;      { licznik w petli sprawdzajacej czy element jest funkcja }
        funkcje:      array[1..liczba_funkcji] of string;
    begin
    ...
    {
      znowu nie ma sposobu na ladna inicjalizacje tablicy funkcji wiec, trzeba inicjalizowac tablice z palca
      jak tu nie kochac pascala...
    }
    funkcje[1] := 'sin';
    funkcje[2] := 'cos';
    funkcje[3] := 'tg';
    ...
    if (czy_ustalony = False ) then  {* funkcja jest jako pusty napis a zapewne bedzie jako tablica napisow, dlatego tutaj
        ...
        begin
        for licz_funk:=1 to liczba_funkcji do
            begin
            if (AnsiCompareText(tymczasowy,funkcje[licz_funk]) = 0 ) then
                begin
